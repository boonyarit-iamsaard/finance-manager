...
import { Separator } from "@finance-manager/ui/components/separator";
import {
  Sheet,
  SheetContent,
  SheetDescription,
  SheetHeader,
  SheetTitle,
} from "@finance-manager/ui/components/sheet";
import { Skeleton } from "@finance-manager/ui/components/skeleton";
import {
  Tooltip,
  TooltipContent,
  TooltipTrigger,
} from "@finance-manager/ui/components/tooltip";
import { useIsMobile } from "@finance-manager/ui/hooks/use-mobile";
...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // biome-ignore lint/suspicious/noDocumentCookie: sidebar open state persisted via cookie per shadcn design
      document.cookie = `${SIDEBAR_COOKIE_NAME}=${openState}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`;
    },
    [setOpenProp, open],
  );

  // biome-ignore lint/correctness/useExhaustiveDependencies: setOpen and setOpenMobile are stable refs
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: all values are stable or already tracked
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "relative flex w-full flex-1 flex-col bg-background md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2 md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-md md:peer-data-[variant=inset]:shadow-sm",
        className,
      )}
      {...props}
    />
